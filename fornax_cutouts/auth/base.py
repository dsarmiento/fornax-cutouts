...
class AbstractAuthProvider(ABC):
    ...
    ``CUTOUTS__SOURCE_PATH``; it is discovered the same way mission sources are.
    """
    ...
    async def resolve(self, request: Request) -> Principal | None:
        ...
        Returning None lets the registry fall through to the next provider (or the
        default anonymous principal) rather than treating the request as unauthenticated.
        """
        ...
